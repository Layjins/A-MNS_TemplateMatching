        ...
        for(int ii=0;ii<DiskNum;ii++)
        {
            ...
            for(i=0;i<Points[ii].size();i++)
            {
                ...
                for(int jj=0;jj<DiskNum;jj++)
                {
                    ...
                    if(tempDist<Thresh_distance)
                    {
                        Pointss[jj]=tempPoint;
                        PointNum++;//第一多的邻居数量
                    }
...
*/
    ...
    MNShsv_SSDtMatching2_Fast(Image,ROI,MatchUpLPoint,MatchTime);//特征：环投影均值、灰度均值和方差、HSV分量均值
...
/*
    ...
    for(int i=1;i<=ClassNum;i++)
    {
        ...
        for(int j=2;j<=num-1;j=j+1)
        {
            ...
            Point MatchUpLPoint[1];
            float MatchTime[1];
            MBNShsv_SSDtMatching2_Fast(Img,TemplateImage,MatchUpLPoint,MatchTime);//特征：环投影均值、灰度均值和方差、HSV分量均值


            //统计匹配时间和匹配率
            MNS_total_SumTime=MNS_total_SumTime+MatchTime[0];
            //匹配率
            Point MatchCenterPoint=MatchUpLPoint[0]+Point((int)(TemplateImage.cols/2),(int)(TemplateImage.rows/2));
            float distance1=get_distance(MatchCenterPoint,TrueCenter[j-2]);
    ...
    float MNS_aver_SumTime=MNS_total_SumTime/ImgNum;
    float MNS_SuccessRate=(float)MNS_SuccessNum/ImgNum;
    float MNS_averSecond_SumTime=MNS_second_SumTime/ImgNum;
    ...
    oFile << "Method" << "," << "Total Images" << "," << "Matching Rate" << "," << "Total Aver Time" << "," << "First Aver Time"<< "," << "Second Aver Time"<< endl;
    oFile << "MNS" << "," << ImgNum << "," << MNS_SuccessRate << "," << MNS_aver_SumTime<< "," << MNS_aver_SumTime-MNS_averSecond_SumTime << "," << MNS_averSecond_SumTime << endl;
    oFile.close();
